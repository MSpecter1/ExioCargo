...
class Transfer:
    def __init__(self, filename, load_containers, unload_containers):
        self.array = self.run(filename, load_containers, unload_containers)

    def getTransferArray(self):
        return self.array

    def run(self, Filename, load_containers, unload_containers):

        filename = Filename

        v = createBasic()
        v = modifyGrid(v, filename)
        offContainers = unload_containers #["Cow"]
        loadContainers = load_containers #["Bat", "Rat"]
